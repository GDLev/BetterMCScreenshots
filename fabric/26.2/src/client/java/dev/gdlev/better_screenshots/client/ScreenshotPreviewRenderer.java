...
public class ScreenshotPreviewRenderer {
    ...
    public static void captureBackground(Runnable onReady) {
        ...
        net.minecraft.client.Screenshot.takeScreenshot(dev.gdlev.better_screenshots.client.MinecraftCompat.mainRenderTarget(mc), image -> mc.execute(() -> {
    ...
    public static boolean handleRenameKey(int action, KeyEvent input) {
        ...
        if (action != InputConstants.PRESS && action != InputConstants.REPEAT) return true;
        ...
        if (previewTexture != null) {
            mc.execute(() -> {
                ScreenshotFullscreenScreen screen =
                        new ScreenshotFullscreenScreen(dev.gdlev.better_screenshots.client.MinecraftCompat.screen(mc));
                ...
                captureBackground(() -> dev.gdlev.better_screenshots.client.MinecraftCompat.setScreen(mc, screen));
            });
        }
    }
