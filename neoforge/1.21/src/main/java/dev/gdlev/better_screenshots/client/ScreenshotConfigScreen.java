...
public class ScreenshotConfigScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        updateWidgetPositions();
        if (doneBtn != null && doneBtn.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
        if (galleryBtn != null && galleryBtn.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
    ...
    private NativeImage scaleTo(NativeImage src, int tw, int th) {
        ...
        for (int y = 0; y < sh; y++) {
            for (int x = 0; x < sw; x++) {
                ...
                int pixel = src.getPixelRGBA(sx, sy);
                dst.setPixelRGBA(ox + x, oy + y, pixel);
            }
        }
        return dst;
    }
