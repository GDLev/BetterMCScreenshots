...
public class ActionButtonConfigScreen extends Screen {
    ...
    private static final Identifier ICON_SHOW =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/show.png");
    private static final Identifier ICON_SHOW_H =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/show_hover.png");
    private static final Identifier ICON_COPY =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/copy.png");
    private static final Identifier ICON_COPY_H =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/copy_hover.png");
    private static final Identifier ICON_UPLOAD =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/upload.png");
    private static final Identifier ICON_UPLOAD_H =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/upload_hover.png");
    private static final Identifier ICON_DELETE =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/delete.png");
    private static final Identifier ICON_DELETE_H =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/delete_hover.png");
    private static final Identifier ICON_RENAME =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/rename.png");
    private static final Identifier ICON_RENAME_H =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/rename_hover.png");
    private static final Identifier ICON_CLOSE =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/close.png");
    private static final Identifier ICON_CLOSE_H =
            Identifier.fromNamespaceAndPath("better_screenshots", "textures/gui/close_hover.png");
    private static final Identifier ICON_PAUSE_SETTINGS =
            Identifier.fromNamespaceAndPath(
                    "better_screenshots", "textures/gui/sprites/icon/settings.png");
    private static final Identifier ICON_PAUSE_GALLERY =
            Identifier.fromNamespaceAndPath(
                    "better_screenshots", "textures/gui/sprites/icon/gallery.png");
    private static final Identifier ICON_PAUSE_SCREENSHOT =
            Identifier.fromNamespaceAndPath(
    ...
    protected void init() {
        ...
        doneButton = addRenderableWidget(Button.builder(
                        ...
                        button -> MinecraftCompat.setScreen(minecraft, parent))
    ...
    public void extractRenderState(
        ...
        for (Button modeButton : modeButtons) {
            if (modeButton != null) modeButton.extractRenderState(context, mouseX, mouseY, delta);
        }
    ...
    private void drawEditorHint(GuiGraphicsExtractor context) {
    ...
    private void drawPlaceholder(GuiGraphicsExtractor context) {
        ...
        if (mode == PreviewMode.PAUSE_MENU) {
            drawPauseMenuReplica(context);
        }
    }

    private void drawPauseMenuReplica(GuiGraphicsExtractor context) {
    ...
    private void drawTray(GuiGraphicsExtractor context) {
        ...
        if (draggingAction >= 0) {
            ...
            context.pose().translate(
                    trayX() + TRAY_W / 2f,
                    trayY() + trayH() / 2f - font.lineHeight * scale / 2f);
            context.pose().scale(scale, scale);
            context.text(font, trash, -trashWidth / 2, 0, 0xFFD45A5A, false);
    ...
    private void drawEmptyTrayText(GuiGraphicsExtractor context) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            context.centeredText(font, Component.literal(lines.get(i)),
                    trayX() + TRAY_W / 2, startY + i * lineHeight, 0xFF8A8A8A);
        }
    }

    private void drawCornerArrows(GuiGraphicsExtractor context) {
        ...
        if (mode == PreviewMode.PAUSE_MENU) {
            int color = dragPauseAnchor == PauseButtonAnchor.CENTER
                    ? 0xFFFFFFFF : 0x77999999;
            context.centeredText(font, Component.literal("☐"),
                    previewX + previewW / 2,
                    previewY + (previewH - font.lineHeight) / 2,
                    color);
        }
    }
    ...
    private void drawActions(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        hoveredAction = -1;
        Identifier[] icons = iconsForMode(false);
        Identifier[] hoverIcons = iconsForMode(true);
        boolean[] visible = visibilityForMode();
        int buttonW = currentButtonWidth();
        int buttonH = currentButtonHeight();
    ...
    private void drawActionTooltip(
        ...
        context.setTooltipForNextFrame(font, label, mouseX, mouseY);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent input, boolean consumed) {
        if (input.button() == 0) {
            ...
            for (int i = 0; i < actionCount(); i++) {
                int x = visible[i] ? actionX[i] : trayActionX[i];
                int y = visible[i] ? actionY[i] : trayActionY[i];
                if (input.x() >= x && input.x() <= x + buttonW
                        && input.y() >= y && input.y() <= y + buttonH) {
                    draggingAction = i;
                    dragMouseX = input.x();
                    dragMouseY = input.y();
                    updateDragTarget(dragMouseX, dragMouseY);
    ...
    private void updateDragTarget(double mouseX, double mouseY) {
        ...
        if (mode == PreviewMode.PAUSE_MENU) {
            if (Math.abs(mouseY
                    - (pauseCenterY() + pauseButtonSize() / 2.0)) <= 18) {
                dragPauseAnchor = PauseButtonAnchor.CENTER;
            } else {
    ...
    private void setPauseAnchor(int action, PauseButtonAnchor anchor) {
        ScreenshotConfig config = ScreenshotConfig.get();
        if (action == PauseMenuButtonLayout.SETTINGS) config.pauseSettingsAnchor = anchor;
        if (action == PauseMenuButtonLayout.GALLERY) config.pauseGalleryAnchor = anchor;
        if (action == PauseMenuButtonLayout.SCREENSHOT) config.pauseScreenshotAnchor = anchor;
    }
    ...
    private void setVisible(int action, boolean visible) {
        ...
        switch (mode) {
            ...
            case PAUSE_MENU -> {
                if (action == PauseMenuButtonLayout.SETTINGS) config.pauseSettingsVisible = visible;
                if (action == PauseMenuButtonLayout.GALLERY) config.pauseGalleryVisible = visible;
                if (action == PauseMenuButtonLayout.SCREENSHOT) config.pauseScreenshotVisible = visible;
            }
        }
    }
    ...
    private void setOrder(int action, int order) {
        ...
        switch (mode) {
            ...
            case PAUSE_MENU -> {
                if (action == PauseMenuButtonLayout.SETTINGS) config.pauseSettingsOrder = order;
                if (action == PauseMenuButtonLayout.GALLERY) config.pauseGalleryOrder = order;
                if (action == PauseMenuButtonLayout.SCREENSHOT) config.pauseScreenshotOrder = order;
            }
        }
    }
    ...
    private Identifier[] iconsForMode(boolean hovered) {
        if (mode == PreviewMode.PAUSE_MENU) {
        ...
        Identifier first = mode == PreviewMode.FULLSCREEN_PREVIEW
