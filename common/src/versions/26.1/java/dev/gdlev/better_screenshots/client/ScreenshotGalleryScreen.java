...
public class ScreenshotGalleryScreen extends Screen {
    ...
    public boolean keyPressed(KeyEvent input) {
        if (editingNameIdx >= 0 && nameEditBox != null) {
            ...
            if (nameEditBox.keyPressed(input)) {
                return true;
            }
        }
    ...
    private void drawSelectedPanel(GuiGraphicsExtractor context) {
        ...
        if (thumbVisible) {
            ...
            Identifier[] icons = { ICON_SHOW, ICON_COPY, ICON_UPLOAD, ICON_DELETE };
            Identifier[] iconsH = { ICON_SHOW_H, ICON_COPY_H, ICON_UPLOAD_H, ICON_DELETE_H };
    ...
    private void extractTopControls(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if (folderButton != null) folderButton.extractRenderState(context, mouseX, mouseY, delta);
    }
    ...
    private void renderNameEditBox(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        if (nameEditBox != null && nameEditBox.isVisible()) {
            nameEditBox.extractRenderState(context, mouseX, mouseY, delta);
        }
    }
