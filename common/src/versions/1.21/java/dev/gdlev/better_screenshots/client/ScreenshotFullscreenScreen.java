...
public class ScreenshotFullscreenScreen extends Screen {
    ...
    private void drawActionButtons(GuiGraphics context,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            context.blit(hovered ? hoverIcons[i] : icons[i],
    ...
    private boolean[] layoutActionButtons(int x, int y, int w, int h, boolean upload) {
        boolean hasFile = !screenshotFiles.isEmpty() && currentFileIndex >= 0;
        ...
        boolean[] visible = {
                ...
                hasFile && config.fullscreenCopyVisible,
                hasFile && upload && config.fullscreenUploadVisible,
                hasFile && config.fullscreenDeleteVisible
        };
        ...
        if (parent instanceof ScreenshotGalleryScreen) {
            ...
            if (bgTex != null && bgTex.getPixels() != null) {
                ...
                if (pixels.getWidth() == this.width && pixels.getHeight() == this.height) {
                    context.blit(ScreenshotPreviewRenderer.BACKGROUND_ID,
